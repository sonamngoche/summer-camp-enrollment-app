...
To run this app on another machine, the person needs Python installed, Streamlit installed, and access to a PostgreSQL database. They would clone the repository to their computer and install the required Python packages, mainly Streamlit and psycopg2-binary. The app uses Streamlit’s secrets system, so they must create a folder called .streamlit in the project directory and add a file named secrets.toml inside it. In that file they need to put their own PostgreSQL connection string under the key DB_URL. The database they connect to must already contain the four tables used by the app: campers, instructors, camps, and camp_enrollments, with the correct schema. After dependencies and secrets are set up, they can run the app by using the command streamlit run 0_Home.py. Streamlit will open the app in a browser window at localhost:8501.

The campers table stores information about each camper, including their first name, last name, age, and guardian contact details. The instructors table stores information about staff members who teach camps, including their names and contact information. The camps table represents each camp session and includes the camp name, description, assigned instructor, start and end dates, and capacity. The camp_enrollments table connects campers to camps. Each row represents one camper enrolled in one camp, and the table prevents duplicate enrollments by using a unique constraint on camper_id and camp_id.
